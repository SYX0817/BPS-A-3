...
#
#         # ✅ 新增的 TransformerEncoder, 用于建模5个packet之间的时序关系
#         encoder_layer = nn.TransformerEncoderLayer(
#             d_model=embed_dim, nhead=2, batch_first=True
#         )
...
#
#         # new_x: (B, 5*21, D)
#         x = new_x.reshape(B, 5, 21, -1)[:, :, 0, :]  # (B, 5, D)，取每个包的 [CLS] token
#
#         # ✅ 加入 Transformer Encoder 提取时序特征
#         x = self.transformer_encoder(x)  # (B, 5, D)
#
#         # 聚合为单个向量
#         x = x.mean(dim=1)  # (B, D)
#
...
class TrafficTransformer(timm.models.vision_transformer.VisionTransformer):
    def __init__(self, **kwargs):
        ...
        # ✅ 新增的 session-level [CLS] token
        self.session_cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))

        # ✅ 用于建模 5 个 packet CLS + 1 个 session CLS 的时序关系
        encoder_layer = nn.TransformerEncoderLayer(
    ...
    def forward_features(self, x):
        ...
        # new_x: (B, 5*21, D) → 取 5 个包的 [CLS]
        x = new_x.reshape(B, 5, 21, -1)[:, :, 0, :]  # (B, 5, D)

        # ✅ 在前面加一个 session-level [CLS] token
        session_cls = self.session_cls_token.expand(B, -1, -1)  # (B, 1, D)
        x = torch.cat((session_cls, x), dim=1)  # (B, 1+5, D)

        # Transformer Encoder 处理
        x = self.transformer_encoder(x)  # (B, 1+5, D)

        # ✅ 取 session CLS (第一个 token) 作为最终表示
        outcome = self.fc_norm(x[:, 0, :])  # (B, D)
        return outcome
